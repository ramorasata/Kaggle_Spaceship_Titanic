...
with open('./01_data/11_raw/test.csv', 'r') as file:
    test_data = pd.read_csv(file, header=0)
...
train_df_optim = train_df[feature_dict[f"{type(estimator).__name__}_8"]]
test_df_optim = test_df[feature_dict[f"{type(estimator).__name__}_8"]]
...
y_test = pd.DataFrame({"PassengerId" : test_data.PassengerId.astype(str),
                       "Transported" : best_model.predict(test_df_optim if feat else test_df)
                       })
